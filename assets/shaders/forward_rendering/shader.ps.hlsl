...
cbuffer PointLight : register(b1, space2)
{
    PointLightData pointLight;
};

cbuffer SpotLight : register(b2, space3)
{
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    // Sample the normal texture (optional if you're doing normal mapping)
    float3 normalColor = NormalTexture.Sample(NormalTextureSampler, input.TexCoord).rgb;

    // Sample the roughness texture (for roughness/metallic properties)
