...
// ======= BEGIN: Texture format mapping to DXGI ======================================
...
// ======= END: Texture format mapping to DXGI ========================================

// ======= BEGIN: Descriptor heap type mapping ========================================
...
// ======= END: Descriptor heap type mapping ==========================================

// ======= BEGIN: Shader binding type mapping =========================================
...
// ======= END: Shader binding type mapping ===========================================
...
// ======= BEGIN: Primitive topology type mapping =====================================
...
// ======= END: Primitive topology type mapping =======================================
...
// ======= END: Resource layout mapping ===============================================

// ======= BEGIN: Texture format REVERSE mapping & functions ==========================

static const std::unordered_map<DXGI_FORMAT, ETextureFormat> textureFormatMappingToGeneric
...
// ======= BEGIN: Descriptor heap type REVERSE mapping & functions ====================

static const std::unordered_map<D3D12_DESCRIPTOR_HEAP_TYPE, EDescriptorHeapTypeDX12> heapTypeReverseMapping
...
EDescriptorHeapTypeDX12 g_getDX12DescriptorHeapType(D3D12_DESCRIPTOR_HEAP_TYPE heapType) {
  return getEnumMapping(heapTypeReverseMapping, heapType, EDescriptorHeapTypeDX12::MAX);
}

// ======= END: Descriptor heap type REVERSE mapping & functions ======================

// ======= BEGIN: Shader binding type REVERSE mapping & functions =====================

static const std::unordered_map<D3D12_DESCRIPTOR_RANGE_TYPE, EShaderBindingType> bindingTypeReverseMapping
...
EShaderBindingType g_getDX12ShaderBindingType(D3D12_DESCRIPTOR_RANGE_TYPE bindingType) {
  return getEnumMapping(bindingTypeReverseMapping, bindingType, EShaderBindingType::MAX);
}

// ======= END: Shader binding type REVERSE mapping & functions =======================

// ======= BEGIN: Texture address mode REVERSE mapping & functions ====================

static const std::unordered_map<D3D12_TEXTURE_ADDRESS_MODE, ETextureAddressMode> addressModeReverseMapping
...
static const std::unordered_map<D3D12_COMPARISON_FUNC, ECompareOp> compareOpReverseMapping
...
static const std::unordered_map<D3D12_STENCIL_OP, EStencilOp> stencilOpReverseMapping 
    = reverseMap(stencilOpMapping);
...
// ======= BEGIN: Primitive topology type REVERSE mapping & functions =================

static const std::unordered_map<D3D12_PRIMITIVE_TOPOLOGY_TYPE, EPrimitiveType> primitiveTopologyTypeReverseMapping
...
EPrimitiveType g_getDX12PrimitiveTopologyTypeOnly(D3D12_PRIMITIVE_TOPOLOGY_TYPE type) {
  return getEnumMapping(primitiveTopologyTypeReverseMapping, type, EPrimitiveType::MAX);
}

// ======= END: Primitive topology type REVERSE mapping & functions ===================

// ======= BEGIN: Primitive topology REVERSE mapping & functions ======================

static const std::unordered_map<D3D_PRIMITIVE_TOPOLOGY, EPrimitiveType> primitiveTopologyReverseMapping
...
EPrimitiveType g_getDX12PrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY type) {
  return getEnumMapping(primitiveTopologyReverseMapping, type, EPrimitiveType::MAX);
}
...
static const std::unordered_map<D3D12_INPUT_CLASSIFICATION, EVertexInputRate> vertexInputRateReverseMapping
...
EVertexInputRate g_getDX12VertexInputRate(D3D12_INPUT_CLASSIFICATION inputRate) {
  return getEnumMapping(vertexInputRateReverseMapping, inputRate, EVertexInputRate::MAX);
}
...
static const std::unordered_map<D3D12_FILL_MODE, EPolygonMode> polygonModeReverseMapping
...
// ======= BEGIN: Cull mode REVERSE mapping & functions ===============================

static const std::unordered_map<D3D12_CULL_MODE, ECullMode> cullModeReverseMapping 
    = reverseMap(cullModeMapping);
...
ECullMode g_getDX12CullMode(D3D12_CULL_MODE cullMode) {
  return getEnumMapping(cullModeReverseMapping, cullMode, ECullMode::MAX);
}

// ======= END: Cull mode REVERSE mapping & functions =================================

// ======= BEGIN: Blend factor REVERSE mapping & functions ============================

static const std::unordered_map<D3D12_BLEND, EBlendFactor> blendFactorReverseMapping 
    = reverseMap(blendFactorMapping);
...
EBlendFactor g_getDX12BlendFactor(D3D12_BLEND type) {
  return getEnumMapping(blendFactorReverseMapping, type, EBlendFactor::MAX);
}
...
static const std::unordered_map<D3D12_BLEND_OP, EBlendOp> blendOpReverseMapping 
    = reverseMap(blendOpMapping);
...
// ======= BEGIN: Resource layout REVERSE mapping & functions =========================

static const std::unordered_map<D3D12_RESOURCE_STATES, EResourceLayout> resourceLayoutReverseMapping
...
EResourceLayout g_getDX12ResourceLayout(D3D12_RESOURCE_STATES resourceState) {
  return getEnumMapping(resourceLayoutReverseMapping, resourceState, EResourceLayout::MAX);
}

// ======= END: Resource layout REVERSE mapping & functions ===========================
