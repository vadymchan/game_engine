...
class AssimpModelLoader : public IModelLoader {
  ...
  std::shared_ptr<Mesh> processMesh(aiMesh* ai_mesh);
  void            processVertices(aiMesh* ai_mesh, std::shared_ptr<Mesh>& mesh);
  ...
  std::pair<math::Vector3Df, math::Vector3Df> processTangentBitangent(
      aiMesh* ai_mesh, unsigned int index);
  math::Vector4Df processColor(aiMesh* ai_mesh, unsigned int index);
  void            processIndices(aiMesh* ai_mesh, std::shared_ptr<Mesh>& mesh);
  void            calculateTangentsAndBitangents(aiMesh*              ai_mesh,
