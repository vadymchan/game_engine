...
#include "gfx/rhi/rhi_new/backends/vulkan/sampler_vk.h"
...
DeviceVk::DeviceVk(const DeviceDesc& desc)
  ...
  if (!createInstance_() || !setupDebugMessenger_() || !createSurface_() || !pickPhysicalDevice_()
      || !createLogicalDevice_() || !createCommandPools_() || !createDescriptorPools_()) {
...
VkBuffer DeviceVk::createStagingBuffer(const void* data, size_t size, VkDeviceMemory& memory) {
  ...
  if (vkCreateBuffer(m_device_, &bufferInfo, nullptr, &stagingBuffer) != VK_SUCCESS) {
    return VK_NULL_HANDLE;
  }

  // Get memory requirements
  VkMemoryRequirements memRequirements;
  vkGetBufferMemoryRequirements(m_device_, stagingBuffer, &memRequirements);
  ...
  if (vkAllocateMemory(m_device_, &allocInfo, nullptr, &memory) != VK_SUCCESS) {
    vkDestroyBuffer(m_device_, stagingBuffer, nullptr);
    return VK_NULL_HANDLE;
  }

  // Bind memory to buffer
  vkBindBufferMemory(m_device_, stagingBuffer, memory, 0);

  // Copy data to buffer
  void* mapped;
  vkMapMemory(m_device_, memory, 0, size, 0, &mapped);
  memcpy(mapped, data, size);
  vkUnmapMemory(m_device_, memory);
...
void DeviceVk::updateBuffer(Buffer* buffer, const void* data, size_t size, size_t offset) {
  ...
  VkBuffer       stagingBuffer = createStagingBuffer(data, size, stagingMemory);

  if (stagingBuffer == VK_NULL_HANDLE) {
    GlobalLogger::Log(LogLevel::Error, "updateBuffer: Failed to create staging buffer");
    return;
  }
  ...
  vkDestroyBuffer(m_device_, stagingBuffer, nullptr);
  vkFreeMemory(m_device_, stagingMemory, nullptr);
}
