...
RenderPassVk::RenderPassVk(const RenderPassDesc& desc, DeviceVk* device)
  ...
  if (!initialize_(desc)) {
    GlobalLogger::Log(LogLevel::Error, "RenderPassVk: Failed to initialize render pass");
  }
}
...
bool RenderPassVk::initialize_(const RenderPassDesc& desc) {
  ...
  if (result != VK_SUCCESS) {
    GlobalLogger::Log(LogLevel::Error, "RenderPassVk::initialize - Failed to create render pass");
    return false;
  }
