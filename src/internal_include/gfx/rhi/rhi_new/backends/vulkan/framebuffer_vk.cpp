...
FramebufferVk::FramebufferVk(const FramebufferDesc& desc, DeviceVk* device)
  ...
  if (!initialize(desc)) {
    GlobalLogger::Log(LogLevel::Error, "FramebufferVk: Failed to initialize framebuffer");
  }
}
...
bool FramebufferVk::initialize(const FramebufferDesc& desc) {
  ...
  for (auto* texture : desc.colorAttachments) {
    ...
    if (!textureVk) {
      GlobalLogger::Log(LogLevel::Error, "FramebufferVk::initialize - Invalid texture type for color attachment");
      return false;
    }
  ...
  if (desc.hasDepthStencil && desc.depthStencilAttachment) {
    ...
    if (!m_depthStencilAttachment) {
      GlobalLogger::Log(LogLevel::Error,
                        "FramebufferVk::initialize - Invalid texture type for depth/stencil attachment");
      return false;
    }
  }
  ...
  if (!renderPassVk) {
    GlobalLogger::Log(LogLevel::Error, "FramebufferVk::initialize - Invalid render pass type");
    return false;
  }
  ...
  if (result != VK_SUCCESS) {
    GlobalLogger::Log(LogLevel::Error, "FramebufferVk::initialize - Failed to create framebuffer");
    return false;
  }
