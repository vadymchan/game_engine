...
bool GraphicsPipelineVk::initialize_() {
  ...
  // Get the RenderPass from our pipeline description
...
bool GraphicsPipelineVk::createShaderStages_(std::vector<VkPipelineShaderStageCreateInfo>& shaderStages) {
  ...
  for (Shader* shader : m_desc_.shaders) {
    ...
    shaderStageInfo.stage                           = g_getShaderStageFlagVk(shaderVk->getStage());
...
bool GraphicsPipelineVk::createPipelineLayout_() {
  ...
  m_descriptorSetLayouts_.clear();
  m_ownedDescriptorSetLayouts_.clear();

  for (const auto& layoutDesc : m_desc_.setLayouts) {
    // Create a descriptor set layout
    auto descriptorSetLayout = m_device_->createDescriptorSetLayout(layoutDesc);
    if (!descriptorSetLayout) {
      GlobalLogger::Log(LogLevel::Error, "Failed to create descriptor set layout for pipeline");
      return false;
    }

    DescriptorSetLayoutVk* layoutVk = dynamic_cast<DescriptorSetLayoutVk*>(descriptorSetLayout.get());
    if (!layoutVk) {
      GlobalLogger::Log(LogLevel::Error, "Invalid descriptor set layout type for Vulkan pipeline");
      return false;
    }

    // Store the native Vulkan handle for later use in pipeline creation
    m_descriptorSetLayouts_.push_back(layoutVk->getLayout());

    // Store the descriptor set layout object to manage its lifetime
    m_ownedDescriptorSetLayouts_.push_back(std::move(descriptorSetLayout));
  }

  // Create push constant ranges if needed (not implemented in this example)

  // Create the pipeline layout
  VkPipelineLayoutCreateInfo pipelineLayoutInfo = {};
  pipelineLayoutInfo.sType                      = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
  pipelineLayoutInfo.setLayoutCount             = static_cast<uint32_t>(m_descriptorSetLayouts_.size());
  pipelineLayoutInfo.pSetLayouts                = m_descriptorSetLayouts_.data();
