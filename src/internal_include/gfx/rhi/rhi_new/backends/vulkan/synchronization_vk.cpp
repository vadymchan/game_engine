...
void SemaphoreVk::signal(VkQueue queue) {
  // In Vulkan, semaphores are signaled through queue operations
  // This is a utility method for simple cases, but normally semaphores
  // are signaled through command buffer submissions or swapchain operations
  VkSubmitInfo submitInfo         = {};
  submitInfo.sType                = VK_STRUCTURE_TYPE_SUBMIT_INFO;
  submitInfo.commandBufferCount   = 0;
