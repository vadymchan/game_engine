...
FramebufferDx12::FramebufferDx12(const FramebufferDesc& desc, DeviceDx12* device)
  ...
  if (!initialize_(desc)) {
    GlobalLogger::Log(LogLevel::Error, "FramebufferDx12: Failed to initialize framebuffer");
  }
}
...
bool FramebufferDx12::initialize_(const FramebufferDesc& desc) {
  ...
  for (auto* texture : desc.colorAttachments) {
    ...
    if (!textureDx12) {
      GlobalLogger::Log(LogLevel::Error, "FramebufferDx12::initialize - Invalid texture type for color attachment");
      return false;
    }
    ...
    if (rtvHandle.ptr == 0) {
      GlobalLogger::Log(LogLevel::Error, "FramebufferDx12::initialize - Texture doesn't have a valid RTV");
      return false;
    }
  ...
  if (desc.hasDepthStencil && desc.depthStencilAttachment) {
    ...
    if (!m_depthStencilAttachment_) {
      GlobalLogger::Log(LogLevel::Error,
                        "FramebufferDx12::initialize - Invalid texture type for depth/stencil attachment");
      return false;
    }
    ...
    if (m_dsvHandle_.ptr == 0) {
      GlobalLogger::Log(LogLevel::Error, "FramebufferDx12::initialize - Texture doesn't have a valid DSV");
      return false;
    }
  }
...
void FramebufferDx12::transitionToRenderTargetState(CommandBufferDx12* cmdBuffer) {
  ...
  for (auto* texture : m_colorAttachments_) {
    ...
    barrier.oldLayout           = ResourceLayout::ShaderReadOnly;  // Assume previous state
...
void FramebufferDx12::transitionToResourceState(CommandBufferDx12* cmdBuffer) {
  ...
  for (auto* texture : m_colorAttachments_) {
    ResourceBarrierDesc barrier = {};
    barrier.texture             = texture;
    barrier.oldLayout           = ResourceLayout::ColorAttachment;
  ...
  if (m_hasDepthStencil_ && m_depthStencilAttachment_) {
    ResourceBarrierDesc barrier = {};
    barrier.texture             = m_depthStencilAttachment_;
    barrier.oldLayout           = ResourceLayout::DepthStencilAttachment;
    barrier.newLayout           = ResourceLayout::ShaderReadOnly;
    cmdBuffer->resourceBarrier(barrier);
  }
}
