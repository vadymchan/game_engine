...
constexpr uint32_t DESCRIPTOR_HEAP_CAPACITY_RTV         = 256;
constexpr uint32_t DESCRIPTOR_HEAP_CAPACITY_DSV         = 256;
constexpr uint32_t DESCRIPTOR_HEAP_CAPACITY_CBV_SRV_UAV = 10'000;
constexpr uint32_t DESCRIPTOR_HEAP_CAPACITY_SAMPLER     = 256;
...
DeviceDx12::DeviceDx12(const DeviceDesc& desc)
  ...
  if (!createFactory_() || !createDevice_() || !createCommandQueue_() || !createCommandPools_()
      || !createDescriptorHeaps_()) {
    // TODO: Handle initialization failure
    // In a real implementation, proper error handling and cleanup would be done
  }
}
...
DeviceDx12::~DeviceDx12() {
  ...
  m_rtvHeap_.release();
  m_dsvHeap_.release();
  m_cbvSrvUavHeap_.release();
  m_samplerHeap_.release();

  // Release main DX12 objects
...
bool DeviceDx12::createDescriptorHeaps_() {
  if (!m_rtvHeap_.initialize(m_device_.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, DESCRIPTOR_HEAP_CAPACITY_RTV, false)) {
    return false;
  }

  if (!m_dsvHeap_.initialize(m_device_.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, DESCRIPTOR_HEAP_CAPACITY_DSV, false)) {
    return false;
  }

  if (!m_cbvSrvUavHeap_.initialize(
          m_device_.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, DESCRIPTOR_HEAP_CAPACITY_CBV_SRV_UAV, true)) {
    return false;
  }

  if (!m_samplerHeap_.initialize(
          m_device_.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, DESCRIPTOR_HEAP_CAPACITY_SAMPLER, true)) {
    return false;
  }
...
std::unique_ptr<CommandBuffer> DeviceDx12::createCommandBuffer(const CommandBufferDesc& desc) {
  ...
  return std::make_unique<CommandBufferDx12>(this, commandList.Get(), commandAllocator);
}
...
void DeviceDx12::updateBuffer(Buffer* buffer, const void* data, size_t size, size_t offset) {
  ...
  if (!bufferDx12) {
    GlobalLogger::Log(LogLevel::Error, "updateBuffer: Invalid buffer type");
    return;
  }

  // Check data
  if (!data || size == 0) {
    GlobalLogger::Log(LogLevel::Warning, "updateBuffer: No data to update");
    return;
  }

  // Check that size + offset does not exceed the buffer size
  if (offset + size > bufferDx12->getSize()) {
    GlobalLogger::Log(LogLevel::Error, "updateBuffer: Update exceeds buffer size");
    return;
  }
