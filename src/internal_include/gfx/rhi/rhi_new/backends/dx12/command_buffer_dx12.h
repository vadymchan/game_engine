...
class CommandBufferDx12 : public CommandBuffer {
  public:
  CommandBufferDx12(DeviceDx12* device, ID3D12GraphicsCommandList* commandList, ID3D12CommandAllocator* commandAllocator);
  ~CommandBufferDx12() = default;
  ...
  /**
   * @note DirectX 12 doesn't have the concept of render passes like Vulkan.
   *       These methods emulate Vulkan's render pass behavior by managing
   *       render target binding, clearing, and resource state transitions
   *       to provide a unified API across backends.
   */
  ...
  ID3D12GraphicsCommandList* getCommandList() const { return m_commandList_; }
  ...
  // Raw pointers to DX12 objects - owned and managed by DeviceDx12
  // This class only uses these resources and does not manage their lifetime
  ID3D12GraphicsCommandList* m_commandList_;
