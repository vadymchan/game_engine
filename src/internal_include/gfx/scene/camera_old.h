...
#ifndef GAME_ENGINE_CAMERA_OLD_H
#define GAME_ENGINE_CAMERA_OLD_H
...
enum class ECameraTypeDeprecated {
  Perspective = 0,
  Orthographic,
  MAX
};

struct CameraParametersDeprecated {
  ...
  float                 m_width;
  float                 m_height;
  ECameraTypeDeprecated m_type;

  // bool            m_isPerspectiveProjection;

  static CameraParametersDeprecated s_fromConfig(const ConfigValue& value);
};
...
class CameraOld {
  ...
  // CameraOld()
  //     : m_type_(ECameraTypeDeprecated::Perspective) {}

  CameraOld(const math::Vector3Df&   pos,
            ...
            float                    width,
            float                    height,
            ECameraTypeDeprecated    type = ECameraTypeDeprecated::Perspective)
      : m_type_(type)
      , m_position_(pos)
  ...
  virtual math::Matrix4f<> createProjection() const {
    ...
    if (m_type_ == ECameraTypeDeprecated::Perspective) {
      if (m_isInfinityFar_) {
  ...
  void updateFromConfig() {
    ...
    if (config) {
      ...
      auto cameraParameters = config->get<CameraParametersDeprecated>("camera");
...
#endif  // GAME_ENGINE_CAMERA_OLD_H
